...
function demotheme_setup() {
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'demotheme_custom_background_args',
			array(
...
function demotheme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'demotheme_content_width', 640 );
}
...
function demotheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'demotheme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'demotheme' ),
...
function demotheme_scripts() {
	wp_enqueue_style( 'demotheme-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'demotheme-style', 'rtl', 'replace' );

	wp_enqueue_script( 'demotheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
